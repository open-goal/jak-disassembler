...
// V2 - not really in use anymore, but V4 will resue logic from it (and the game didn't rename the functions)
// V3 - optimized for code and small stuff. Supports segments (main, debug, top-level)
// V4 - optimized for data (never code) and big stuff, special optimization possible for large V4 objects at the end of DGO.
...
// Header for link data used for V2, V3, V4 objects.  For V3/V4, this is found at the beginning of the object data.
...
static uint32_t c_symlink2(LinkedObjectFile& f, const std::vector<uint8_t>& data, uint32_t code_ptr_offset, uint32_t link_ptr_offset, SymbolLinkKind kind, const char* name) {
  ...
  do {
    ...
    // link table has a series of variable-length-encoded integers indicating the seek amount to hit each reference
    // to the symbol.  It ends when the seek is 0, and all references to this symbol have been patched.
 ...
 */
static uint32_t c_symlink3(LinkedObjectFile& f, const std::vector<uint8_t>& data, uint32_t code_ptr, uint32_t link_ptr, SymbolLinkKind kind, const char* name, int seg) {
...
static void link_v4(LinkedObjectFile& f, const std::vector<uint8_t>& data) {
  ...
  const uint8_t* code_end = &data.at(code_offset + code_size); // safe because link data is after code.
  ...
  if(data.at(link_ptr_offset) == 0) {
    ...
    // there are pointers.
    // there are a series of variable-length coded integers, indicating where the pointers are, in the form:
    // seek_amount, number_of_consecutive_pointers, seek_amount, number_of_consecutive_pointers, ... , 0
    ...
    while(true) { // loop over entire table
      while(true) { // loop over current mode (fixing/seeking)
        ...
        // when we "end" an encoded integer on an 0xff, we need an explicit zero byte to change modes.
        // this handles this special case.
        if(data.at(link_ptr_offset) == 0) {
          link_ptr_offset++;
          fixing = !fixing;
        }
...
//    printf(" [%d] %d %d %d %d\n", i, header->segment_info[i].size, header->segment_info[i].data, header->segment_info[i].magic, header->segment_info[i].relocs);
  }
